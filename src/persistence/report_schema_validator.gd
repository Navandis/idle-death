...
static func validate(data: Variant, gameplay_cursor_msec: int) -> Dictionary:
	...
	for index in range(data.history.size()):
		result = _validate_record(data.history[index], cursor.value, previous_report_sequence, previous_snapshot_time, previous_event_sequence, seen_event_sequences, "report_state.history.%d" % index); if not result.ok: return result
	...
	result = _validate_accumulator(data.live, cursor.value, true, previous_event_sequence, seen_event_sequences, "report_state.live"); if not result.ok: return result
...
static func _validate_record(data: Variant, report_cursor: int, previous_sequence: int, previous_snapshot_time: int, previous_event_sequence: int, seen_event_sequences: Dictionary, path: String) -> Dictionary:
	...
	result = _validate_accumulator(data.window, snapshot_time.value, false, previous_event_sequence, seen_event_sequences, path + ".window"); if not result.ok: return result
	if result.end_time != snapshot_time.value: return _err(ERR_CROSS_FIELD, path + ".window.window_ended_simulation_msec")
	return {"ok": true, "code": OK, "sequence": sequence.value, "snapshot_time": snapshot_time.value, "event_sequence": result.event_sequence}

static func _validate_accumulator(data: Variant, expected_end: int, live_window: bool, previous_event_sequence: int, seen_event_sequences: Dictionary, path: String) -> Dictionary:
	if typeof(data) != TYPE_DICTIONARY: return _err(ERR_TYPE, path)
	...
	for index in range(data.recent_events.size()):
		result = _validate_event(data.recent_events[index], start.value, end.value, last_event_sequence, seen_event_sequences, path + ".recent_events.%d" % index); if not result.ok: return result
	...
	return {"ok": true, "code": OK, "end_time": end.value, "event_sequence": last_event_sequence}
...
static func _validate_event(data: Variant, window_start: int, window_end: int, previous_sequence: int, seen_event_sequences: Dictionary, path: String) -> Dictionary:
	if typeof(data) != TYPE_DICTIONARY: return _err(ERR_TYPE, path)
