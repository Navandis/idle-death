...
static func validate(data: Variant, gameplay_cursor_msec: int) -> Dictionary:
	...
	result = _validate_accumulator(data.live, cursor.value, true, "report_state.live"); if not result.ok: return result
	var previous_sequence := 0
	for index in range(data.history.size()):
		result = _validate_record(data.history[index], cursor.value, previous_sequence, "report_state.history.%d" % index); if not result.ok: return result
		previous_sequence = int(result.sequence)
	return {"ok": true, "code": OK, "cursor": cursor.value}

static func _validate_record(data: Variant, report_cursor: int, previous_sequence: int, path: String) -> Dictionary:
	...
	if snapshot_time.value > report_cursor: return _err(ERR_CROSS_FIELD, path + ".snapshot_simulation_msec")
	result = _validate_accumulator(data.window, snapshot_time.value, false, path + ".window"); if not result.ok: return result
	if result.end_time != snapshot_time.value: return _err(ERR_CROSS_FIELD, path + ".window.window_ended_simulation_msec")
	return {"ok": true, "code": OK, "sequence": sequence.value}

static func _validate_accumulator(data: Variant, expected_end: int, live_window: bool, path: String) -> Dictionary:
	if typeof(data) != TYPE_DICTIONARY: return _err(ERR_TYPE, path)
	...
	for key in data.attribution_slices.keys():
		...
		result = _validate_slice(data.attribution_slices[key], key, path + ".attribution_slices." + key); if not result.ok: return result
	...
	for index in range(data.recent_events.size()):
		result = _validate_event(data.recent_events[index], start.value, end.value, previous_sequence, path + ".recent_events.%d" % index); if not result.ok: return result
	...
	return {"ok": true, "code": OK, "end_time": end.value}

static func _validate_slice(data: Variant, expected_key: String, path: String) -> Dictionary:
	if typeof(data) != TYPE_DICTIONARY: return _err(ERR_TYPE, path)
	...
	if start.value > end.value or end.value - start.value != elapsed.value: return _err(ERR_CROSS_FIELD, path + ".elapsed_msec")
	...
	for key in data.channel_summaries_by_channel_id.keys():
		...
		result = _validate_channel(data.channel_summaries_by_channel_id[key], key, threshold.value, path + ".channel_summaries_by_channel_id." + key); if not result.ok: return result
...
static func _validate_channel(data: Variant, expected_channel: String, expected_threshold: String, path: String) -> Dictionary:
	if typeof(data) != TYPE_DICTIONARY: return _err(ERR_TYPE, path)
	...
	if int(data.total_banked_units_end) < int(data.total_banked_units_start) or int(data.total_banked_units_end) - int(data.total_banked_units_start) != int(data.banked_units_delta): return _err(ERR_CROSS_FIELD, path + ".banked_units_delta")
	return {"ok": true, "code": OK}

static func _validate_event(data: Variant, window_start: int, window_end: int, previous_sequence: int, path: String) -> Dictionary:
	if typeof(data) != TYPE_DICTIONARY: return _err(ERR_TYPE, path)
	...
	if sequence.value <= previous_sequence: return _err(ERR_CROSS_FIELD, path + ".event_sequence")
	...
	return {"ok": true, "code": OK, "sequence": sequence.value, "event_type": type_value.value}
