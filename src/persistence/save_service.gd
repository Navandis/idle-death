...
func save_snapshot(snapshot: Dictionary) -> Dictionary:
	var validation := SaveSchemaValidator.validate_v4(snapshot)
...
func load_snapshot() -> Dictionary:
	...
	return {"ok": true, "code": OK, "snapshot": valid[0].snapshot, "save_revision": valid[0].save_revision, "selected_role": valid[0].role, "diagnostics": diagnostics, "migration_required": valid[0].get("migration_required", false), "migration_from_version": valid[0].get("migration_from_version", SaveEnvelope.CURRENT_SCHEMA_VERSION)}
...
func _read_candidate(path: String, role: String) -> Dictionary:
	...
	return {"ok": true, "role": role, "path": path, "snapshot": migrated.snapshot, "save_revision": validation.save_revision, "migration_required": migrated.get("migrated", false), "migration_from_version": schema_parse.value}
