...
func _validate_terms(terminology: CoreTerminologyDefinition) -> void:
	...
	for term in terminology.terms:
		...
		if term.fallback_text.strip_edges() == "":
			_add_error(terminology.resource_path, "terms.%s.fallback_text" % term.id, "non-empty", "empty")
		_terms[term.id] = {"id": term.id, "fallback_text": term.fallback_text, "localization_key": term.localization_key}
...
func _normalize_channel(record: Dictionary, definition: OutputChannelDefinition) -> void:
	...
	record["starts_hidden"] = definition.starts_hidden
	record["requires_progress_display"] = definition.requires_progress_display
...
func _normalize_guarantee(record: Dictionary, definition: GuaranteeDefinition) -> void:
	...
	record["floor_quantity"] = definition.floor_quantity
	record["source_cost_ids"] = definition.source_cost_ids.duplicate()
...
func _normalize_modifiers(modifiers: Array, owner: Resource, field: String) -> Array:
	...
	for modifier in modifiers:
		...
		result.append({"metric": modifier.metric, "operation": modifier.operation, "scope": modifier.scope, "condition": modifier.condition, "value_subunits": _normalize_decimal(modifier.value, owner, field + ".value"), "target_id": modifier.target_id})
	return result
...
func _validate_channel_record(record: Dictionary, records: Dictionary) -> void:
	...
	if record.requires_progress_display and record.output_kind != "WHOLE_ITEM": _add_error(record.source_path, "requires_progress_display", "whole item channel", record.output_kind)
...
func _validate_guarantee_record(record: Dictionary, records: Dictionary) -> void:
	...
	if record.floor_quantity <= 0: _add_error(record.source_path, "floor_quantity", "positive", str(record.floor_quantity))
