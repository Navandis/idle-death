...
function Assert-WorkflowStateExactKeys {
    ...
    $actual = @(Get-WorkflowStatePropertyNames $Value | Sort-Object)
    $wanted = @($Expected | Sort-Object)
    if (($actual.Count -ne $wanted.Count) -or (@(Compare-Object $actual $wanted).Count -ne 0)) { throw "$Path has unexpected or missing fields." }
}
...
function Assert-WorkflowStateUri {
    ...
    if ($null -ne $Value) { $uri = $null; if (-not [uri]::TryCreate($Value, [UriKind]::Absolute, [ref]$uri) -or $uri.AbsoluteUri -ne $Value) { throw "$Path must be a normalized absolute URI." } }
}

function Assert-WorkflowStateSha { param($Value, [string]$Path) Assert-WorkflowStateString $Value $Path $true $false; if ($null -ne $Value -and $Value -notmatch '^[0-9a-f]{40}$') { throw "$Path must be a lowercase 40-character SHA." } }
...
function Test-WorkflowStateSensitiveError { param([string]$ErrorText) return $ErrorText -match '(?i)(authorization|bearer|token|hosts\.yml|gho_[a-z0-9_]+|github_pat_[a-z0-9_]+)' }
...
function Assert-WorkflowStateQuery {
    ...
    if ($scope -notin @('local','github','invariant','schema')) { throw "$Path.scope is invalid." }
...
function Test-WorkflowStateContract {
    ...
    $parsedTimestamp = [DateTimeOffset]::MinValue; if (-not [DateTimeOffset]::TryParse($timestamp, [ref]$parsedTimestamp) -or $timestamp -notmatch 'Z$') { throw 'generated_at_utc must be RFC3339 UTC.' }
    $result = Get-WorkflowStateProperty $Document 'result'; if ($result -notin @('pass','partial','fail')) { throw 'result is invalid.' }
    ...
    foreach ($name in @('status_entries','staged_paths')) { $items = Get-WorkflowStateProperty $workspace $name; Assert-WorkflowStateArray $items "workspace.$name"; foreach ($item in $items) { Assert-WorkflowStateString $item "workspace.$name item" $false $false } }
    ...
    $threads = Get-WorkflowStateProperty $github 'review_threads'; Assert-WorkflowStateExactKeys $threads @('total_count','unresolved_count','items') 'github.review_threads'; $total = Get-WorkflowStateProperty $threads 'total_count'; $unresolved = Get-WorkflowStateProperty $threads 'unresolved_count'; Assert-WorkflowStateInteger $total 'github.review_threads.total_count' 0 ([Int64]::MaxValue); Assert-WorkflowStateInteger $unresolved 'github.review_threads.unresolved_count' 0 ([Int64]::MaxValue); $items = Get-WorkflowStateProperty $threads 'items'; Assert-WorkflowStateArray $items 'github.review_threads.items'; if ($items.Count -ne $total -or $unresolved -gt $total) { throw 'github.review_threads counts do not match items.' }; $seenThreadIds = @{}; $recount = 0; foreach ($item in $items) { Assert-WorkflowStateExactKeys $item @('id','is_resolved','is_outdated','path','line') 'github.review_threads item'; Assert-WorkflowStateString (Get-WorkflowStateProperty $item 'id') 'github.review_threads.id' $false $true; $threadId = Get-WorkflowStateProperty $item 'id'; if ($seenThreadIds.ContainsKey($threadId)) { throw 'github.review_threads IDs must be unique.' }; $seenThreadIds[$threadId] = $true; Assert-WorkflowStateBoolean (Get-WorkflowStateProperty $item 'is_resolved') 'github.review_threads.is_resolved'; Assert-WorkflowStateBoolean (Get-WorkflowStateProperty $item 'is_outdated') 'github.review_threads.is_outdated'; Assert-WorkflowStateString (Get-WorkflowStateProperty $item 'path') 'github.review_threads.path' $true $false; Assert-WorkflowStateInteger (Get-WorkflowStateProperty $item 'line') 'github.review_threads.line' 1 ([Int64]::MaxValue) $true; if (-not (Get-WorkflowStateProperty $item 'is_resolved')) { $recount++ } }; if ($recount -ne $unresolved) { throw 'github.review_threads unresolved_count does not match items.' }

    $queries = Get-WorkflowStateProperty $Document 'queries'; Assert-WorkflowStateArray $queries 'queries'; $failed = @(); $seenQueries = @{}; foreach ($query in $queries) { Assert-WorkflowStateQuery $query 'queries item'; $key = "$(Get-WorkflowStateProperty $query 'scope')`u001f$(Get-WorkflowStateProperty $query 'name')"; if ($seenQueries.ContainsKey($key)) { throw 'queries must not duplicate scope/name pairs.' }; $seenQueries[$key] = $true; if (-not (Get-WorkflowStateProperty $query 'ok')) { $failed += $query } }
    if ($result -eq 'pass' -and $failed.Count -ne 0) { throw 'pass must contain no failed query.' }
    if ($result -eq 'partial' -and (($failed.Count -eq 0) -or @($failed | Where-Object { (Get-WorkflowStateProperty $_ 'scope') -ne 'github' }).Count -ne 0)) { throw 'partial requires only failed GitHub queries.' }
    if ($result -eq 'fail' -and @($failed | Where-Object { (Get-WorkflowStateProperty $_ 'scope') -in @('local','invariant','schema') }).Count -eq 0) { throw 'fail requires a failed local, invariant, or schema query.' }
    if ($result -in @('pass','partial')) { if ([string]::IsNullOrWhiteSpace((Get-WorkflowStateProperty $repository 'root')) -or [string]::IsNullOrWhiteSpace((Get-WorkflowStateProperty $repository 'branch')) -or $null -eq (Get-WorkflowStateProperty $repository 'head') -or $null -eq (Get-WorkflowStateProperty $repository 'origin_main') -or (Get-WorkflowStateProperty $repository 'origin_fetch_urls').Count -eq 0 -or (Get-WorkflowStateProperty $repository 'origin_push_urls').Count -eq 0 -or [string]::IsNullOrWhiteSpace((Get-WorkflowStateProperty $workspace 'index_lock_path'))) { throw 'pass and partial require complete local-state evidence.' } }
    return $true
}
