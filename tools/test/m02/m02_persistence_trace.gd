...
func _init() -> void:
	...
	_check(service.save_runtime(GameState.new(100), TimeAuthorityState.new(), 1, "prototype-content-r1").ok, "revision 1 written")
	_check(service.save_runtime(GameState.new(200), TimeAuthorityState.new(), 2, "prototype-content-r1").ok, "revision 2 written")
