...
try {
    ...
    $fullOk = Invoke-NativeStep 'full-gut' $PowerShellExe @('-NoProfile', '-ExecutionPolicy', 'Bypass', '-File', $wrapperPath, '-GodotBin', $resolvedGodot) { param($output, $exitCode) Test-ExactCounts $output 193 193 3659 }
    ...
    $focusedOk = Invoke-NativeStep 'focused-r1' $PowerShellExe @('-NoProfile', '-ExecutionPolicy', 'Bypass', '-Command', $focusedCommand) { param($output, $exitCode) Test-ExactCounts $output 15 15 827 }
    ...
    Write-Log "Full GUT: 193/193 tests, 3659 assertions ($script:FullGutSummary)"
    Write-Log "Focused R1: 15/15 tests, 827 assertions ($script:FocusedSummary)"
