...
try {
    ...
    $fullOk = Invoke-NativeStep 'full-gut' $PowerShellExe @('-NoProfile', '-ExecutionPolicy', 'Bypass', '-File', $wrapperPath, '-GodotBin', $resolvedGodot) { param($output, $exitCode) Test-ExactCounts $output 194 194 4408 }
    ...
    $focusedOk = Invoke-NativeStep 'focused-r1' $PowerShellExe @('-NoProfile', '-ExecutionPolicy', 'Bypass', '-Command', $focusedCommand) { param($output, $exitCode) Test-ExactCounts $output 16 16 1576 }
    ...
    Write-Log "Full GUT: 194/194 tests, 4408 assertions ($script:FullGutSummary)"
    Write-Log "Focused R1: 16/16 tests, 1576 assertions ($script:FocusedSummary)"
