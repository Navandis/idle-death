...
try {
    ...
    $fullOk = Invoke-NativeStep 'full-gut' $PowerShellExe @('-NoProfile', '-ExecutionPolicy', 'Bypass', '-File', $wrapperPath, '-GodotBin', $resolvedGodot) { param($output, $exitCode) Test-ExactCounts $output 187 187 2886 }
    ...
    $focusedOk = Invoke-NativeStep 'focused-r1' $PowerShellExe @('-NoProfile', '-ExecutionPolicy', 'Bypass', '-Command', $focusedCommand) { param($output, $exitCode) Test-ExactCounts $output 9 9 54 }
    ...
    Write-Log "Full GUT: 187/187 tests, 2886 assertions ($script:FullGutSummary)"
    Write-Log "Focused R1: 9/9 tests, 54 assertions ($script:FocusedSummary)"
