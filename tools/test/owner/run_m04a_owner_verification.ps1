# Canonical owner-run Windows verification package for M04A.
param(
    [string]$GodotBin,
    [string]$CommitSha = ""
)
...
$ExitCode = 0
$CleanupResult = "PASS"
$PendingInteractiveChecks = "None for M04A"
...
$LastStepOutput = @()
$TraceRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("death-idle-m04a-" + [System.Guid]::NewGuid().ToString("N"))
...
$LogPath = Join-Path $LogDir "M04A-owner-verification-$Stamp-$LogRef.log"
...
$FocusedGutArgs = @("-gdir=res://tests/unit/m04a", "-gdir=res://tests/integration/m04a")
...
try {
    ...
    if ($GodotVersionPassed) {
        ...
        Run-Step "Focused M04A" "tools\test\run_gut.ps1 -GodotBin <resolved> -GutArgs @('-gdir=res://tests/unit/m04a','-gdir=res://tests/integration/m04a')" { Invoke-GutWrapper $FocusedGutArgs }
        ...
        Write-SkippedStep "Focused M04A" "prerequisite failed: Godot version validation"
