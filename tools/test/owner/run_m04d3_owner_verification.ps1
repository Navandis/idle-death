# Canonical owner-run Windows verification package for M04D3.
param(
    [string]$GodotBin,
    [string]$CommitSha = ""
)
...
$ExitCode = 0
$CleanupResult = "PASS"
$PendingInteractiveChecks = "None for M04D3"
...
$LastStepOutput = @()
$TraceRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("death-idle-m04d3-" + [System.Guid]::NewGuid().ToString("N"))
...
$LogPath = Join-Path $LogDir "M04D3-owner-verification-$Stamp-$LogRef.log"
...
function Run-Step([string]$Name, [string]$CommandDescription, [scriptblock]$Command) {
    ...
    if ($Name -eq "Focused M04D3") {
        ...
        if (($JoinedOutput -match "Passing Tests\s+none") -or ($JoinedOutput -notmatch "Passing Tests\s+[1-9]")) {
            Write-LogLine "ERROR: Focused M04D3 GUT output did not report any passing tests."
            $StepExitCode = 1
        }
    }
...
$FocusedGutArgs = @("-gdir=res://tests/unit/m04d3", "-gdir=res://tests/integration/m04d3")
...
try {
    ...
    if ($GodotVersionPassed) {
        ...
        if ($FullBeforePassed) {
            $FocusedPassed = Run-Step "Focused M04D3" "tools\test\run_gut.ps1 -GodotBin <resolved> -GutArgs @('-gdir=res://tests/unit/m04d3','-gdir=res://tests/integration/m04d3')" { Invoke-GutWrapper $FocusedGutArgs }
            if ($FocusedPassed) {
                ...
                if ($ImportPassed) {
                    $TracePassed = Run-Step "Trace" "$ResolvedGodot --headless --path $RepoRoot -s res://tools/test/m04d3/m04d3_rate_context_trace.gd -- --save-root $TraceRoot" { Invoke-GodotCommand @("--headless", "--path", "$RepoRoot", "-s", "res://tools/test/m04d3/m04d3_rate_context_trace.gd", "--", "--save-root", "$TraceRoot") }
                    $CapturedTraceOutput = @($script:LastStepOutput)
                    if ($TracePassed) { Run-Step "Trace-result marker verification" "verify required TRACE M04D3 markers" { Invoke-TraceMarkerVerification -TraceOutput $CapturedTraceOutput } }
                ...
                Write-SkippedStep "Import" "prerequisite failed: Focused M04D3"
                Write-SkippedStep "Trace" "prerequisite failed: Focused M04D3"
                Write-SkippedStep "Trace-result marker verification" "prerequisite failed: Focused M04D3"
            }
        }
        else {
            Write-SkippedStep "Focused M04D3" "prerequisite failed: Full GUT before"
        ...
        Write-SkippedStep "Focused M04D3" "prerequisite failed: Godot version validation"
