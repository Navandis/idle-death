# Canonical owner-run Windows verification package for M04D1.
param(
    [string]$GodotBin,
    [string]$CommitSha = ""
)
...
$ExitCode = 0
$CleanupResult = "PASS"
$PendingInteractiveChecks = "None for M04D1"
...
$LastStepOutput = @()
$TraceRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("death-idle-m04d1-" + [System.Guid]::NewGuid().ToString("N"))
...
$LogPath = Join-Path $LogDir "M04D1-owner-verification-$Stamp-$LogRef.log"
...
function Run-Step([string]$Name, [string]$CommandDescription, [scriptblock]$Command) {
    ...
    if ($Name -eq "Focused M04D1") {
        ...
        if (($JoinedOutput -match "Passing Tests\s+none") -or ($JoinedOutput -notmatch "Passing Tests\s+[1-9]")) {
            Write-LogLine "ERROR: Focused M04D1 GUT output did not report any passing tests."
            $StepExitCode = 1
        }
    }
...
$FocusedGutArgs = @("-gdir=res://tests/unit/m04d1", "-gdir=res://tests/integration/m04d1")
...
try {
    ...
    if ($GodotVersionPassed) {
        ...
        if ($FullBeforePassed) {
            $FocusedPassed = Run-Step "Focused M04D1" "tools\test\run_gut.ps1 -GodotBin <resolved> -GutArgs @('-gdir=res://tests/unit/m04d1','-gdir=res://tests/integration/m04d1')" { Invoke-GutWrapper $FocusedGutArgs }
            if ($FocusedPassed) {
                ...
                if ($ImportPassed) {
                    $TracePassed = Run-Step "Trace" "$ResolvedGodot --headless --path $RepoRoot -s res://tools/test/m04d1/m04d1_output_access_trace.gd -- --save-root $TraceRoot" { Invoke-GodotCommand @("--headless", "--path", "$RepoRoot", "-s", "res://tools/test/m04d1/m04d1_output_access_trace.gd", "--", "--save-root", "$TraceRoot") }
                    $CapturedTraceOutput = @($script:LastStepOutput)
                    if ($TracePassed) { Run-Step "Trace-result marker verification" "verify required TRACE M04D1 markers" { Invoke-TraceMarkerVerification -TraceOutput $CapturedTraceOutput } }
                ...
                Write-SkippedStep "Import" "prerequisite failed: Focused M04D1"
                Write-SkippedStep "Trace" "prerequisite failed: Focused M04D1"
                Write-SkippedStep "Trace-result marker verification" "prerequisite failed: Focused M04D1"
            }
        }
        else {
            Write-SkippedStep "Focused M04D1" "prerequisite failed: Full GUT before"
        ...
        Write-SkippedStep "Focused M04D1" "prerequisite failed: Godot version validation"
