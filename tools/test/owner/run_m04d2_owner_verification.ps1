...
$ExitCode = 0
$CleanupResult = "PASS"
$PendingInteractiveChecks = "None for M04D2"
...
$LastStepOutput = @()
$TraceRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("death-idle-m04d2-" + [System.Guid]::NewGuid().ToString("N"))
...
$LogPath = Join-Path $LogDir "M04D2-owner-verification-$Stamp-$LogRef.log"
...
function Run-Step([string]$Name, [string]$CommandDescription, [scriptblock]$Command) {
    ...
    if ($Name -eq "Focused M04D2") {
        ...
        if (($JoinedOutput -match "Passing Tests\s+none") -or ($JoinedOutput -notmatch "Passing Tests\s+[1-9]")) {
            Write-LogLine "ERROR: Focused M04D2 GUT output did not report any passing tests."
            $StepExitCode = 1
        }
    }
...
$FocusedGutArgs = @("-gdir=res://tests/unit/m04d2", "-gdir=res://tests/integration/m04d2")
...
try {
    ...
    if ($GodotVersionPassed) {
        ...
        if ($FullBeforePassed) {
            $FocusedPassed = Run-Step "Focused M04D2" "tools\test\run_gut.ps1 -GodotBin <resolved> -GutArgs @('-gdir=res://tests/unit/m04d2','-gdir=res://tests/integration/m04d2')" { Invoke-GutWrapper $FocusedGutArgs }
            if ($FocusedPassed) {
                ...
                if ($ImportPassed) {
                    $TracePassed = Run-Step "Trace" "$ResolvedGodot --headless --path $RepoRoot -s res://tools/test/m04d2/m04d2_channel_accumulation_trace.gd -- --save-root $TraceRoot" { Invoke-GodotCommand @("--headless", "--path", "$RepoRoot", "-s", "res://tools/test/m04d2/m04d2_channel_accumulation_trace.gd", "--", "--save-root", "$TraceRoot") }
                    $CapturedTraceOutput = @($script:LastStepOutput)
                    if ($TracePassed) { Run-Step "Trace-result marker verification" "verify required TRACE M04D2 markers" { Invoke-TraceMarkerVerification -TraceOutput $CapturedTraceOutput } }
                ...
                Write-SkippedStep "Import" "prerequisite failed: Focused M04D2"
                Write-SkippedStep "Trace" "prerequisite failed: Focused M04D2"
                Write-SkippedStep "Trace-result marker verification" "prerequisite failed: Focused M04D2"
            }
        }
        else {
            Write-SkippedStep "Focused M04D2" "prerequisite failed: Full GUT before"
        ...
        Write-SkippedStep "Focused M04D2" "prerequisite failed: Godot version validation"
