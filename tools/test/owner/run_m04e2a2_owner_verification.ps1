# Canonical exact-head Windows owner package for M04E2A2.
param(
    [string]$GodotBin,
    [string]$CommitSha = ""
)
...
$TraceRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("death-idle-m04e2a2-" + [System.Guid]::NewGuid().ToString("N"))
$FailedStepCount = 0
$ExitCode = 0
$CleanupResult = "PASS"
...
function Write-SummaryAndExit([int]$Code) {
    ...
    Write-LogLine "Pending interactive checks: None for M04E2A2"
...
$LogPath = Join-Path $LogDir ("M04E2A2-owner-verification-{0}-{1}.log" -f $UtcStart.ToString("yyyyMMdd-HHmmssZ"), $LogRef)
...
if (-not [string]::IsNullOrWhiteSpace($CommitSha) -and ($DetectedCommit -eq "unavailable" -or $DetectedCommit -ne $CommitSha)) {
...
$Focused = @(
    "-gdir=res://tests/unit/m04a", "-gdir=res://tests/unit/m04d1", "-gdir=res://tests/unit/m04e2t1", "-gdir=res://tests/unit/m04e2t2", "-gdir=res://tests/unit/m04e2a2",
    "-gdir=res://tests/unit/persistence", "-gdir=res://tests/integration/m04a", "-gdir=res://tests/integration/m04d1", "-gdir=res://tests/integration/m04e2t1", "-gdir=res://tests/integration/m04e2t2", "-gdir=res://tests/integration/m04e2a2", "-gdir=res://tests/integration/save_load"
)
...
try {
    ...
    $FullBefore = $false; $FocusedOk = $false; $ImportOk = $false; $TraceOk = $false
    if ($GodotOk) { $FullBefore = Run-Step "Full GUT before" "tools\test\run_gut.ps1" { Invoke-Gut } } else { Skip-Step "Full GUT before" "Godot validation failed" }
    if ($FullBefore) { $FocusedOk = Run-Step "Focused M04E2A2 persistence suites" "tools\test\run_gut.ps1 with M04E2A2 focus" { Invoke-Gut $Focused } } else { Skip-Step "Focused M04E2A2 persistence suites" "full GUT before failed" }
    if ($FocusedOk) { $ImportOk = Run-Step "Explicit import" "$ResolvedGodot --headless --path <repo> --import" { Invoke-Godot @("--headless", "--path", "$RepoRoot", "--import") } } else { Skip-Step "Explicit import" "focused suite failed" }
    if ($ImportOk) {
        $TraceOutput = @()
        $TraceOk = Run-Step "M04E2A2 trace" "$ResolvedGodot --headless --path <repo> -s m04e2a2_report_schema_trace.gd -- --work-root <isolated>" { $script:TraceOutput = @( & $ResolvedGodot --headless --path "$RepoRoot" -s "res://tools/test/m04e2a2/m04e2a2_report_schema_trace.gd" -- --work-root "$TraceRoot" ); $TraceOutput }
        if ($TraceOk) { Run-Step "Exact 15-marker verification" "verify all M04E2A2 markers" { Verify-TraceMarkers $TraceOutput } } else { Skip-Step "Exact 15-marker verification" "trace failed" }
    } else { Skip-Step "M04E2A2 trace" "import failed"; Skip-Step "Exact 15-marker verification" "import failed" }
    Run-Step "Missing-root trace must fail" "$ResolvedGodot --headless ... m04e2a2_report_schema_trace.gd (no --work-root)" {
        $oldPreference = $ErrorActionPreference; $ErrorActionPreference = "Continue"
        try { & $ResolvedGodot --headless --path "$RepoRoot" -s "res://tools/test/m04e2a2/m04e2a2_report_schema_trace.gd"; $negativeExit = $LASTEXITCODE }
