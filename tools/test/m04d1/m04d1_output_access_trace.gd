...
func _run_trace() -> void:
	...
	var inventory_before := state.inventory.deep_clone()
	var time_before := state.simulation_time_msec
	...
	_assert(state.simulation_time_msec == time_before and _inventory_equal(state.inventory, inventory_before), "no time or inventory backfill")
	for threshold in state.thresholds.values():
		for channel_id in threshold.channel_acquisition.keys():
			...
			_assert(a.progress_subunits == 0 and a.rate_carry_units == 0 and a.total_banked_units == 0, "no progress/carry/banked backfill")
	...
	_assert(not state.progression.unlocked_output_item_ids.has(&"RES_PROVISIONS") and provisions.size() == 1 and not persisted.game_state.progression.has("insight"), "access knowledge insight separate")
...
func _inventory_equal(a, b) -> bool:
