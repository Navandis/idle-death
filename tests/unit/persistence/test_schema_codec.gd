...
func test_schema_rejects_cross_field_contradictions_and_unknown_codec() -> void:
	...
	assert_eq(SaveSchemaValidator.validate_v1(s).code, SaveSchemaValidator.ERR_CROSS_FIELD)
	s = _snapshot(false)
	s.codec_id = "OTHER"
	assert_eq(SaveSchemaValidator.validate_v1(s).code, SaveSchemaValidator.ERR_CODEC)
