...
func test_query_eta_percentage_and_display() -> void:
	...
	assert_eq(query.disclosure_state, "IDENTIFIED")
...
func test_signature_mismatch_matrix_and_no_mutation() -> void:
	...
	for field in form_fields.keys():
		...
		var continuity := ReapingRateContextService.new(registry).compare_residual_signatures(state, &"THR_GLOAMWOOD", &"FORM_MAN_AT_ARMS", &"FORM_SCRIBE")
		assert_eq(continuity.normalization_required_fields, [field], field)
		assert_eq(continuity.mismatched_fields, continuity.normalization_required_fields, field)
