...
func test_first_second_save_backup_and_highest_revision_selection() -> void:
	assert_true(service.save_runtime(GameState.new(1), TimeAuthorityState.new(), 1).ok)
	assert_true(storage.exists(files.primary_path))
	assert_true(service.save_runtime(GameState.new(2), TimeAuthorityState.new(), 2).ok)
	...
	storage.files[files.backup_path] = JsonSaveCodec.new().encode(SaveSchemaMapper.runtime_to_snapshot(GameState.new(3), TimeAuthorityState.new(), 3)).bytes
...
func test_failure_injection_leaves_previous_valid_snapshot() -> void:
	...
	for operation in ["write", "copy", "rename"]:
		...
		assert_false(service.save_runtime(GameState.new(2), TimeAuthorityState.new(), 2).ok, operation)
		var loaded := service.load_runtime()
...
func test_invalid_existing_primary_is_preserved_before_replacement() -> void:
	...
	assert_true(service.save_runtime(GameState.new(5), TimeAuthorityState.new(), 1).ok)
