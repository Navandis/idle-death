...
- M04E2R1 merged through PR #34 from final head `ec3fc115cc685d378b93f6968f0f28300760c755` at merge commit `bda72749cfc6f20bfa1fd8e42acefdb6e3a8f31d`; CI run 55, review and triage convergence, exact-head Windows owner verification, and post-merge synchronization and branch cleanup completed. M04E2R2 is in progress under its owner-approved v0.3 packet and draft PR; M04E2P1/M04E2B remain deferred.
...
### Current M04E2 status after `DEC-0045`
...
The current executable/planning sequence is:

```text
M04E2T1 -> M04E2T2 -> M04E2R1 -> M04E2R2 -> M04E2P1 -> M04E2B
```

`DEC-0046` is Accepted and M04E2R1 is Merged/Passed through PR #34 at merge commit `bda72749cfc6f20bfa1fd8e42acefdb6e3a8f31d` after exact-head CI, review convergence, and Windows owner verification. M04E2R2 is in progress under its owner-approved v0.3 packet and draft PR. Before P1 the report ledger remains explicitly caller-owned and non-persisted; no application, `GameSession`, service member, autoload, singleton, or hidden global may retain canonical mutable ledger state. P1 must introduce only cursor-aligned exposed/persisted `GameState`; B may hold lag only privately between simulation and ingestion, and begins and ends aligned. M04E2P1 and M04E2B remain deferred.
...
M00 → M01 → M02 → M03
                      └─ GATE-GAMEPLAY-SCHEMA
                           └─ M04A → M04B → M04C → M04D1 → M04D2 → M04D3 → M04E1 → M04E2T1 → M04E2T2 → M04E2A2 → M04E2A3 → M04E2A4 → M04E2B
```
...
The predecessor decomposition immediately above is superseded by `DEC-0045` for its M04E2 portion. The current M04 completion route is M04E2T1 -> M04E2T2 -> M04E2R1 -> M04E2R2 -> M04E2P1 -> M04E2B.
...
After M04A, M04B, M04C, M04D1, M04D2, M04D3, M04E1, M04E2T1, M04E2T2, M04E2R1, M04E2R2, M04E2P1, and M04E2B pass, one Gloamwood Reaping led by Man-at-Arms can be constructed, dispatched, advanced deterministically, saved, loaded, recalled, redispatched, forecast on a clone, atomically committed into report state, snapshotted into bounded history, and reloaded without claim-gating. Output access, backlog, Essence, Mastery, and configured eligible channels use scene-independent authority.

#### Epic completion rule

M04 is complete only when M04A, M04B, M04C, M04D1, M04D2, M04D3, M04E1, M04E2T1, M04E2T2, M04E2R1, M04E2R2, M04E2P1, and M04E2B are all Merged and Passed. No `M04-persistent-reaping-simulation.md`, direct `M04D` prompt, direct `M04E-forecast-report-simulation-harness.md` prompt, or direct M04E2 prompt is valid under the approved recalibration.
...
The predecessor decomposition immediately above is superseded by `DEC-0045` for its M04E2 portion. M04E now completes through M04E2T1 -> M04E2T2 -> M04E2R1 -> M04E2R2 -> M04E2P1 -> M04E2B.
...
### M04E2 — Transaction provenance, report persistence, and final M04 closure
...
**Implementation status:** Not directly executable  
**Verification status:** —  
**Approved decomposition:** M04E2T1 -> M04E2T2 -> M04E2R1 -> M04E2R2 -> M04E2P1 -> M04E2B
...
Accepted `DEC-0043` replaces the implementation packaging in `DEC-0042` with a single-provenance simulation transaction prerequisite followed by finalized typed facts. Accepted `DEC-0045` then retired the former A2/A3/A4 slices after PR #23 and established the runtime-ledger-first R1/R2/P1/B route. `DEC-0046` is Accepted. M04E2R1 merged through PR #34 at `bda72749cfc6f20bfa1fd8e42acefdb6e3a8f31d` and passed exact-head CI, review convergence, Windows owner verification, and integration cleanup. M04E2R2 is in progress under its owner-approved v0.3 packet and draft PR; P1/B remain deferred.

#### Completion rule

M04E2 is not complete. M04E2T1, M04E2T2, and M04E2R1 are Merged/Passed; M04E2R2 is in progress under its owner-approved v0.3 packet and draft PR, while M04E2P1 and M04E2B remain deferred and not started. Completion still requires all six current slices to be Merged and Passed.
...
- M04E2T2 and M04E2R1 completed through PR #22 and PR #34. M04E2R2 is in progress under its owner-approved v0.3 packet and draft PR; M04E2P1 and M04E2B remain deferred and require their own approved packets.
...
**Prompt status:** Implemented — historical v0.1; do not re-execute  
**Implementation status:** Merged through PR #22  
**Verification status:** Passed  
**Executed prompt file:** `docs/codex/milestone-prompts/M04E2T2-finalized-typed-run-facts.md`

#### Realized purpose

M04E2T2 replaced the transitional raw public simulation grammar with one detached typed fact family projected only from finalized M04E2T1 context/journal evidence. Every current simulation/run/debug/test/trace consumer migrated without reopening candidate mutation or commit semantics.

#### Realized boundary

- Global `SimulationResult` with four closed result kinds and exact request/commit cursors.
- Typed historical `SimulationSegmentResult` values including backlog endpoints.
- Typed `SimulationChannelDeltaResult` values including rate period and endpoint totals.
- Closed current event union for channel banking and Settlement.
- Pure `SimulationResultProjector` with no live/candidate `GameState` input.
- Deep detachment, complete value equality, and self-contained structural validation.
- Removal of raw public segment/channel dictionaries, generic payload dictionaries, nested engine result/event ownership, and simulation `change_summary`.
- Direct migration of engine, transaction, run service, debug, tests, traces, and persistence exclusion.
- Schema version 3 and `prototype-content-r2` preserved.

#### Merge and review record
...
Exact-head Windows verification detected the final head and passed:
...
The final slice used 25 non-documentation/non-`.uid` files and 1,460 net additions. The net count exceeded the approximate planning threshold by 10 lines. The owner approved a slice-specific exception because the excess was the approved typed family, explicit edge-case regressions, trace, owner runner, and direct compatibility removal. No new authority, schema transition, later subsystem, or gameplay formula owner entered M04E2T2.

`GATE-FINALIZED-RUN-FACTS` and the M04E2T2 scope gate are satisfied.
...
### M04E2A — Report-foundation implementation sequence
...
The former M04E2A sequence is never executable or completable; it is retained only as historical evidence.
...
Introduce the authoritative, save-owned report-state family and schema version 4. Prove exact runtime construction, deep isolation, validation, primitive mapping, sequential migration, and atomic persistence without adding a report mutator or read API.

No new decision is proposed. The exact state and schema semantics are already maintained in `DATA_AND_CONTENT_CONTRACTS.md`; implementation must stop for an owner-approved decision if those semantics must change.

#### Dependencies and gates

- M04E2T2 Merged and Passed through PR #22.
- M04E2A2 planning and prompt v0.1 reviewed and explicitly owner-approved.
- `GATE-REPORT-SCHEMA` satisfied.
- `GATE-SLICE-SCOPE` satisfied.
- New Codex desktop task and feature branch from current `main`.

#### Included scope

- One typed report-state aggregate family rooted at `ReportState` inside `GameState`.
- Typed live accumulator, immutable record, loadout identity, attribution slice, channel summary, and event detail state.
- Complete deep clone/copy isolation and runtime/content validation.
- Schema version 4 with one exact report-state tree and canonical decimal-string integers.
- Frozen v1/v2/v3 validators and fixture bytes retained.
- Pure `v3 -> v4` migration that creates canonical empty report state at the source simulation cursor and fabricates no history.
- Sequential `v1 -> v2 -> v3 -> v4` proof.
- Populated-v4 mapper parity, codec round trip, real-file persistence, current-v4 no-rewrite, and failure preservation.
- Table-driven runtime and primitive malformed-state matrices.
- Field-by-field construction/clone/copy/map/load propagation proof.
- Focused tests, one isolated real-file trace, and one exact-head Windows owner package.

#### Explicit non-goals

- `ReportService` or another production report mutator.
- Committed-result ingestion or interval classification.
- Public live/history read models.
- Snapshot, clear, archive, or history/event pruning commands.
- Offline report classification or trusted-time orchestration.
- Atomic simulation/report coordination or final M04 harness.
- Changes to simulation, typed result facts, formulas, schema history, codec, content revision, UI, tutorial, progression, Halls, support, analytics, or concurrency.

#### Completion boundary

A valid empty or fixture-populated report state can construct, validate, deep-clone, copy, serialize, encode, decode, load, and migrate exactly. Version-3 saves begin prospective report tracking at their existing simulation cursor; valid current version-4 saves do not rewrite. No production service mutates report state.

#### Scope assessment

| Assessment item | Draft limit |
|---|---|
| Primary subsystem owner | One report-state aggregate/validator family |
| Principal transition | Runtime report state -> schema-v4 mapping/migration/persistence |
| New authoritative aggregate | One `ReportState` family |
| Save-schema/content change | One v3-to-v4 transition; content unchanged |
| Cross-layer seams | 2: GameState/validation; report state/persistence |
| Risk dimensions | State/alias validation; wire/migration compatibility; atomic persistence |
...
| Mandatory stop | Before 34 files or approximately 1,650 lines, another owner/schema, third seam, or report-service mutation |
| Interactive owner checks | None |

#### Follow-on dependency

- M04E2A3 after M04E2A2 is Merged/Passed and receives a fresh ingestion/scope review.
...
Consume finalized typed committed facts into the live report accumulator exactly once without presentation queries or archive operations.
...
- One public report mutator for ingestion.
...
- Historical attribution exclusively from M04E2T2 facts.
- Exact boundary event ownership and no-mutation failures.

#### Explicit non-goals

- Public reads, snapshot command, offline classification, history pruning, final trace/owner runner, UI, M04E2B, or trusted time.

#### Completion boundary

A finalized committed interval is represented once in live report state or rejected transactionally according to the approved decision table.
...
- Pure detached global, Threshold, assignment, lifecycle/channel, and archived-record views.
- Snapshot reasons and complete-cursor/expected-sequence guards.
- Foreground/archive/offline-only isolation and mixed-mode rejection.
- Bounded event detail and recent history with exact compaction counters.
- Snapshot no-gameplay-mutation proof, schema-v4 production round trip, truthful real-file trace, owner runner, artifact audit, and synchronized evidence.

#### Explicit non-goals

- Report UI, claim behavior, partial clears, history deletion, permanent analytics, trusted time, tutorial/progression/Hall/support/service report sources, concurrency, or M04E2B.

#### Completion boundary

The accepted M04E2A report foundation is Merged/Passed only after this slice passes.
...
- M04E2T1, M04E2T2, M04E2R1, M04E2R2, and M04E2P1 all Merged and Passed.
- Fresh prompt and scope approval.

#### Preliminary included scope

- One narrow `SimulationReportCoordinator` using the stable simulation/run and report-ingestion owners.
- Deep-clone, simulate, ingest, validate, and copy live state once.
- Return exact finalized run facts, report result, and one checkpoint request.
- Reject forecast mode and preserve both gameplay/report state on every failure.
- Compare forecast gameplay with equivalent atomic reported commit.
- Snapshot sequence 1, prove gains preexist report inspection, and round-trip through schema v4.

#### Explicit non-goals

- New report fields, report UI, trusted time, tutorial, progression, Halls, support, concurrent Reapings, or platform behavior.

#### Follow-on dependencies

- M05 decomposition and prompt drafting.
