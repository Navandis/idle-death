...
M04D3 implementation note (PR #15 final correction): focused Linux coverage is `tests/unit/m04d3` plus `tests/integration/m04d3` (18 tests, 240 assertions), the full suite is 141 tests and 2,165 assertions, and the real-file trace prints all sixteen approved markers only after behavioral assertions, including real old-context production before a supported rate-context change and the exact `1 -> 3 -> 2 -> 1` owner sequence. Missing, blank, and `user://` trace roots fail before save access; the owner package logs UTF-8 exact-head evidence and remains pending for Windows final-head execution.
