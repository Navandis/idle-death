...
M04D3 implementation note (PR #15 final correction): focused Linux coverage is `tests/unit/m04b`, `tests/unit/m04d3`, plus `tests/integration/m04d3` (34 tests, 571 assertions), the full suite is 143 tests and 2,201 assertions, and the real-file trace prints all sixteen approved markers only after behavioral assertions, including real old-context production before a supported rate-context change, the exact `1 -> 3 -> 2 -> 1` owner sequence, and the assignment guard that rejects unknown nonzero forward-compatible flow residuals before redispatch activation. Missing, blank, and `user://` trace roots fail before save access; the owner package logs UTF-8 exact-head evidence and remains pending for Windows final-head execution.
