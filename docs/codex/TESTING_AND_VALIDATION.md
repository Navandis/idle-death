...
**Document status:** M04E2R1 Merged/Passed through PR #34; `DEC-0046` and `DEC-0047` Accepted, with M04E2R2 implementation in progress through PR #35 and Partial verification
...
## M04D1 completion record

M04D1 merged through PR #13 on 2026-07-18 at merge commit `4569fbfb968deb73f54ba453834dd7af0b04d545` from final head `1171785562c4107921437339187a1e782fe0143b`.

Final evidence:

- full Windows suite before and after: `108/108` tests and `1,544` assertions;
- focused M04D1 suite: `14/14` tests and `193` assertions;
- explicit import: pass;
- schema-v3 pure/sequential migration, content finalization, no-rewrite, failure preservation, access/source validation, command contract, and canonical no-backfill matrices: pass;
- all 12 exact trace markers: pass;
- cleanup, cleanup proof, and artifact audit: pass;
- failed step count `0`;
- pending interactive checks: none;
- final review surface: 33 changed files, 1,196 additions, 48 deletions.
...
## 26. M04D3 completed loadout/rate-context/query validation package
...
```text
Linux/Codex focused M04B + M04D3 + integration: 35/35 tests, 592 assertions
Windows focused M04D3: 18/18 tests, 254 assertions
Windows full before and after: 144/144 tests, 2222 assertions
Import: PASS
Trace markers: 16/16
Cleanup: PASS
Cleanup proof: PASS
...
### 26.2 Completed behavior groups

The final package verifies:

1. assembly-time and commit-time candidate validation;
2. component identity independent from numerical output;
3. supported residual-preserving redispatch;
4. canonical normalization-required diagnostics;
5. unknown-nonzero residual activation rejection;
6. validated empty-Retinue redispatch activation;
7. baseline-derived Form channel modifiers and authored operand normalization;
8. equal-output/different-loadout identity preservation;
9. non-compounding repeated/reversed redispatch;
10. exact progress, carry-aware ETA, and readable three-component display;
11. initialized-source disclosure and null display when ETA is unavailable;
12. real old-context then new-context production;
13. exact `1 -> 3 -> 2 -> 1` operation/source identity;
14. rate-change-boundary chunk equivalence;
15. schema-v3 production persistence and derived-artifact exclusion;
16. source ownership and final branch-recovery workflow.

### 26.3 Final trace markers
...
## 27. M04E1 completed forecast-clone validation package
...
```text
Windows full GUT before: 153/153 tests, 2522 assertions
Windows focused M04E1: 9/9 tests, 295 assertions
Import: PASS
Trace markers: 15/15
Cleanup: PASS
Cleanup proof: PASS
Artifact audit: PASS
Windows full GUT after: 153/153 tests, 2522 assertions
Failed step count: 0
Automated result: PASS
```
...
### 27.2 Completed groups

The final package verifies:

1. fresh-cache global-class registration through canonical import;
2. bounded run modes and complete public result fields;
3. forecast baseline non-mutation;
4. complete bidirectional clone detachment;
5. one-hour exact core/channel fixture;
6. eight-hour exact core/channel fixture;
7. forecast versus separately committed clone canonical equality;
8. Settlement-boundary state/event equality;
9. foreground/offline-fixture/debug mode equality;
10. debug adapter delegation;
11. regular/irregular chunk equality;
12. zero-duration and failure/no-mutation behavior;
13. exact engine summary/segments/deltas/events;
14. generic Broken Watch resource/Whole-Soul channel passthrough;
15. production persistence load and unchanged source save bytes;
16. schema-v3 exclusion of run/projection/result/report artifacts;
17. no clock, scene, platform, report, tutorial, checkpoint, or alternate-formula ownership.

### 27.3 Final trace markers
...
## 29. M04E2T1 single-provenance transaction completion record
...
The realized package preserved schema version 3, content revision `prototype-content-r2`, formulas, public compatibility result representation, and current M04C through M04E1 outcomes while adding the single-provenance transaction/context/journal boundary.

Final exact-head Windows evidence recorded:

- detected Git commit matched `a4d8056cb8771e84e1948fc5e59939c46a13003c`;
- full GUT before and after: `165/165` tests, `2,641` assertions, exit `0`;
- focused M04C through M04E2T1: `61/61` tests, `1,136` assertions, exit `0`;
- explicit Godot import: pass;
- all 12 exact M04E2T1 markers: pass;
- missing-root trace: required exit `1`, correctly treated as pass;
...
`DEC-0046` and `DEC-0047` are Accepted. M04E2R1 is Merged/Passed through PR #34. M04E2R2 is in progress through PR #35 under its owner-approved v0.3 packet, with Partial verification; exact-head Windows owner verification remains pending. The current R1/R2 validation packages apply to their respective boundaries; M04E2P1 and M04E2B remain deferred.
...
Before G3 approval, the R1 interval-decision table and test oracle were pending owner approval. The approved R1 package requires ingestion of finalized committed run facts exactly once into an explicitly caller-owned, non-persisted ledger, with fact-based attribution and no mutation for inputs the approved oracle classifies as rejected or malformed.

R2 validation must prove snapshot, bounded retention, detached reads, and lifecycle transitions on the same explicit ledger. Neither R1 nor R2 may introduce persistence, a hidden owner, or an application coordinator.
...
R1 validates the caller-owned, non-persisted ledger only: factory/clone/validator grammar; wrapper precedence and interval matrix; normalized source/channel/Settlement continuity; one-shot/chunked equality; transactionality and overflow; and schema-v3 persistence exclusion. Required Codex evidence is focused R1 GUT coverage, the full `tools/test/run_gut.sh` suite, import, main-scene smoke, deterministic trace, exact 37-path audit, and artifact cleanup. The Windows owner runner verifies the exact SHA with no interactive checklist; its M04E2R2 exact-head run remains pending while PR #35 is in progress. M04E2P1 and M04E2B remain deferred.
